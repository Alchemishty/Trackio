...
with app.app_context():
    ...
    def signup():
        if request.method == 'POST':
            ...
            if username not in [i.username for i in User.query.all()]:
                ...
                db.session.commit()
                return login()
            return redirect('/notfound/Another user with the same username already exists.')
    ...
    @current_app.route("/notfound/<error>")
    def notfound(error):
    ...
    @current_app.route('/tracker/add', methods=['GET', 'POST'])
    @login_required
    def add_tracker():
        if request.method == 'POST':
            ...
            add = tracker(user_id=u_id, name=name,
                          desc=desc, type=type, settings=set)
            try:
                db.session.add(add)
                db.session.commit()
                return main()
    ...
    @current_app.route('/tracker/<int:tracker_id>', methods=['GET', 'POST'])
    @login_required
    def view_tl(tracker_id):
        ...
        tl = log.query.filter(
            log.tracker_id == tracker_id).order_by(log.log_datetime)
        x, y = [], []
    ...
    @current_app.route('/tracker/<int:tracker_id>/update', methods=['GET', 'POST'])
    @login_required
            ...
            try:
                if request.form.get('name') != t.name or request.form.get('desc') != t.desc or request.form.get('type') != request.form.get('settings'):
                    db.session.query(log).filter(
                        log.tracker_id == tracker_id).delete()
                updict = {tracker.name: request.form['name'], tracker.desc: request.form['desc'],
                          tracker.type: request.form['type'], tracker.settings: request.form['settings']}
                print(updict)
                db.session.query(tracker).filter(
                    tracker.tracker_id == tracker_id).update(updict)
                db.session.commit()
                return main()
            except:
    ...
    @current_app.route('/tracker/<int:tracker_id>/delete', methods=['GET', 'POST'])
    @login_required
    ...
    @current_app.route('/<int:tracker_id>/log/add', methods=['GET', 'POST', 'PUT'])
    @login_required
        ...
        if request.method == 'POST':
            try:
                ...
                if t.type == 'Time':
                    ...
                    l = log(tracker_id=tracker_id, log_datetime=log_datetime,
                            note=request.form.get('note'), log_value=value)
                    db.session.add(l)
                    db.session.commit()
    ...
    @current_app.route('/<int:log_id>/log/update', methods=['GET', 'POST'])
    @login_required
    def update_log(log_id):
        ...
        if request.method == 'POST':
            ...
            log_datetime = datetime.strptime(
                request.form.get("time"), '%d/%b/%Y, %H:%M:%S.%f')
            db.session.query(log).filter(log.log_id == log_id).update(
                {'log_value': log_value, 'note': log_note, 'log_datetime': log_datetime})
            db.session.commit()
            return view_tl(l.tracker_id)
        return render_template('update_logs.html', datetime=datetime, log=l)

    @current_app.route('/<int:log_id>/log/delete', methods=['GET', 'POST'])
    @login_required
